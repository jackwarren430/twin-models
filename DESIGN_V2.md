...
| **Template collapse under a non-thinking creator** (one trivial statement template, coefficient swaps, prompt-example parroting) | OBSERVED (mini-04b, all 15 iters; STOPPED by pre-registered rule). Mechanism: GRPO only reinforces what sampling discovers; without deliberation, sampled diversity collapses, and trivial+consistent (rgrad floor 0.267 + full consistency) is a local optimum — consistency risk makes hard problems unprofitable to attempt. **Rotation TRANSPORTS the collapse**: the incoming creator reproduces the distribution it was trained solving (B converged in ONE update). Mitigations: bounded deliberation (thinking ON + hard `creator_max_tokens` cap), `w_diversity` on the **digit-normalized** similarity (raw bigrams were dodged by coefficient swaps), example-reuse ban in prompts, grounded themes — all bundled in mini-05 |
